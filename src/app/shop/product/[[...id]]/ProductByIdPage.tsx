...
import {
    getProductSlugFromParams,
    getProductSlugFromPathname,
} from "@/utils/productSlug";

const ProductByIdPage = () => {
    const params = useParams();
    const paramSlug = getProductSlugFromParams(
        params as { id?: string | string[] },
    );
    const [productSlug, setProductSlug] = useState(paramSlug);
    ...
    useEffect(() => {
        if (paramSlug) {
            setProductSlug(paramSlug);
            return;
        }
        setProductSlug(getProductSlugFromPathname(window.location.pathname));
    }, [paramSlug]);
    ...
    if (!product) {
        return (
            <div className="max-w-6xl mx-auto px-4 md:px-6 py-10 min-h-screen text-center text-gray-600">
                {language === "hi"
                    ? "उत्पाद नहीं मिला।"
                    : "Product not found."}
            </div>
    ...
    const handleAddToCart = async () => {
        ...
        try {
            const cartSlug = product.slug || productSlug;
            const sendQty = isYagya ? 1 : qty;
            await addToCart(cartSlug, String(sendQty));
            setAdded(true);
